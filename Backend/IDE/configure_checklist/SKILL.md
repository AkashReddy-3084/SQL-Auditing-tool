...
| File | Effect |
|------|--------|
| `Backend/checklists/custom-checklist.json` | The approved item is appended under its Area/Sub-area. |
| `Backend/checklists/custom-deterministic-script-mapping.json` | `script_file`, `scope`, `IsAdminCheck`, `IsDocumentationCheck`, `MCP_Feasibility` for the new ID. |
| `Backend/checklists/Scripts/sql/<id>.sql` | The approved script (when one is feasible). |
...
| `Backend/checklists/custom-checklist-pending.json` | Reserved-but-unapproved drafts; entries are removed on approval or rejection. |
