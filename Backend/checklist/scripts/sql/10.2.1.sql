...
-- Scope: DATABASE
-- Scoring: 3 = enabled (READ_WRITE); 2 = enabled (READ_ONLY); 1 = enabled (OFF); 0 = disabled or error
...
BEGIN
    ...
    SELECT DB_NAME(),
           CASE 
               WHEN actual_state = 1 AND operation_mode = 1 THEN 3 
               WHEN actual_state = 1 AND operation_mode = 2 THEN 2 
               WHEN actual_state = 1 AND operation_mode = 0 THEN 1 
               ELSE 0 
           END,
           'actual_state = ' + CAST(actual_state AS VARCHAR(1)) + ', operation_mode = ' + 
           CASE operation_mode 
               WHEN 0 THEN 'OFF' 
