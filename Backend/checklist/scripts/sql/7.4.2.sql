...
-- Scope: SERVER
-- Scoring: 2 = SQL Audit or trigger evidence captures login/permission actions; 1 = partial capture evidence; 0 = no capture evidence
...
DECLARE @Finding NVARCHAR(MAX) = N'Login and permission logging metadata could not be evaluated';
...
BEGIN TRY
    ...
    SET @Score = CASE WHEN @ServerAudits > 0 AND @AuditSpecs > 0 AND @AclActions > 0 THEN 2 WHEN @AuditSpecs > 0 OR @AclActions > 0 OR @ServerTriggers > 0 THEN 1 ELSE 0 END;
    SET @Finding = N'server_audits=' + CONVERT(NVARCHAR(20), @ServerAudits) + N', audit_specs=' + CONVERT(NVARCHAR(20), @AuditSpecs) + N', acl_audit_actions=' + CONVERT(NVARCHAR(20), @AclActions) + N', server_triggers=' + CONVERT(NVARCHAR(20), @ServerTriggers);
END TRY
BEGIN CATCH
    SET @Score = 0;
    SET @Finding = N'Unable to read login and permission logging metadata: ' + ERROR_MESSAGE();
END CATCH;
