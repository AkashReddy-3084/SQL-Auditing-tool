    ...
    public partial class MainWindow : Window
    {
        ...
        private async void LoadChecklistBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // Items the engine keeps out of the script pipeline: a documentation check has no
                // script at all, an admin check has one the operator runs. Mirrors Auditor.IsScriptMapped.
                var manualOnlyItems = new System.Collections.Generic.HashSet<string>(StringComparer.OrdinalIgnoreCase);
                // Mapping paths are relative to the repo root, not to whatever directory the app
                // was launched from, so the root found here is what the file probe below must use.
                        ...
                        if (File.Exists(candidate))
                        {
                            ...
                            foreach (var prop in mapDoc.RootElement.EnumerateObject())
                            {
                                ...
                                else if (prop.Value.ValueKind == System.Text.Json.JsonValueKind.Object)
                                {
                                    ...
                                    if ((prop.Value.TryGetProperty("IsDocumentationCheck", out var docCheck) && docCheck.ValueKind == System.Text.Json.JsonValueKind.True)
                                        || (prop.Value.TryGetProperty("IsAdminCheck", out var adminCheck) && adminCheck.ValueKind == System.Text.Json.JsonValueKind.True))
                                    {
                                        manualOnlyItems.Add(prop.Name);
                                    }
                                }
                            }
                            break;
                ...
                {
                    var scriptRoot = mappingRoot ?? Directory.GetCurrentDirectory();
                    foreach (var it in _loadedItems)
                    {
                        if (manualOnlyItems.Contains(it.Id)) continue;
                        if (mappingFile.TryGetValue(it.Id, out var files) && files != null && files.Length > 0)
                        {
                            ...
                            foreach (var f in files.Where(s => !string.IsNullOrWhiteSpace(s)))
                            {
                                ...
                                {
                                    var candidate = f;
                                    if (!Path.IsPathRooted(candidate)) candidate = Path.Combine(scriptRoot, candidate.Replace('/', Path.DirectorySeparatorChar));
